...
int main() {
    ...
    const int TRIES = 500;
    const int MAX_RANDOM_MOVES = 50;
    const int STUCK_LIMIT = 5;
    ...
    while(running) {
        ...
        cout << totalCost(cube, MN) << " " << stuck << endl;
        ...
        if (stuck > STUCK_LIMIT) {
            ...
            if (poinSebelumnya < lowestValue) lowestValue = poinSebelumnya;
        }

        if (randomMoves > MAX_RANDOM_MOVES) {
            cout << "Search berhenti pada " << (t - STUCK_LIMIT) << " iterasi dengan sisa cost " << lowestValue << endl;
            cout << "RANDOM MOVES " << randomMoves << endl;
            running = false;
        }
